...
PARAMETER_ATOM = ( ATTRIBUTE_REFERENCE | PARAMETER_NAME | STATIC_REFRENCE )

//here I simply try to keep close to the original grammar as much as possible but bottom line ... the grammar arround expr is broken ...
EXPR = PARAMETER_ATOM
...
ATTRIBUTE_DEFINITION = ATTRIBUTE_NAME >> ":=" >> ( STATIC_REFERENCE | EXPR )

ATTRIBUTE_DEFINITIONS = ( ATTRIBUTE_DEFINITION % ',' )                /// 3


CONSUMING_EVENTS = (EVENT_NAME % ',')                                 /// 4

/// FINAL
RULE = DEFINE >> EVENT_DEFINITION >> FROM >> TRIGGER_PATTERNS >> -(WHERE ATTRIBUTE_DEFINITIONS) >> -(CONSUMING CONSUMING_EVENTS) >> ';'
...
typedef boost::variant< attribute_reference, std::string, static_value  > parameter_atom;
...
struct tesla_rule{
  ...
  boost::optional<std::vector<attribute_definition> > attribute_definitions_;
